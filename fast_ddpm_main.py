...
def parse_args_and_config():   
    ...
    if args.config.startswith('configs/') or args.config.startswith('configs\\'):
...
def main():
    ...
    try:   
        ...
        use_wavelet = getattr(config.model, 'use_wavelet', False)
        
        if args.sample:   
            if use_wavelet:
                if config.model.type == 'wavelet_sg':
                    runner.wavelet_sg_sample()
                else:
                    raise NotImplementedError(f"Wavelet sampling not implemented for model type {config.model.type}")
            else:
            ...
            if use_wavelet:
                if config.model.type == 'wavelet_sg':
                    runner.wavelet_sg_train()
                else:
                    raise NotImplementedError(f"Wavelet training not implemented for model type {config.model.type}")
            else:
