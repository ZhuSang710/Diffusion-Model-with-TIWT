...
def wavelet_sg_noise_estimation_loss(model,
                                   x_img_wavelet: torch.Tensor,
                                   x_gt_wavelet: torch.Tensor,
                                   t: torch.LongTensor,
                                   e: torch.Tensor,
                                   b: torch.Tensor, keepdim=False):
    ...
    # 在小波域添加噪声
    x_wavelet = x_gt_wavelet * a.sqrt() + e * (1.0 - a).sqrt()
    
    # 模型预测（在小波域）
    output = model(torch.cat([x_img_wavelet, x_wavelet], dim=1), t.float())
...
loss_registry = {
    ...
    'wavelet_sg': wavelet_sg_noise_estimation_loss
}
